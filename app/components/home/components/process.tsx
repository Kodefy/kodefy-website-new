      ...
      <div className="mx-auto grid max-w-360 gap-14 px-6 py-20 sm:px-8 lg:grid-cols-[1fr_1fr] lg:gap-24 lg:px-12 lg:py-28">
        <div className="lg:sticky lg:top-12 lg:self-start">
          <h2 className="max-w-md text-5xl leading-none font-light tracking-tight sm:text-6xl">
            <RevealHeadline
              revealBy="character"
              text={content.processIntro.title}
            />
          </h2>
          <FadeInText
            className="mt-4 max-w-md text-base leading-7 text-muted-foreground sm:text-lg sm:leading-8"
          ...
          <div className="relative mt-2 aspect-4/5 w-full max-w-lg lg:max-w-145">
            <Image
              ...
              alt={
                locale === "id"
                  ? "Mockup tampilan website Kodefy di ponsel"
        ...
        <div>
          <FadeIn className="flex justify-start lg:justify-end" delay={0.3}>
          ...
          <ol className="mt-14 lg:mt-20">
            {content.process.map((step, index) => (
              <SlideFadeIn
                key={step.number}
                as="li"
                className="grid gap-5 border-t border-black/15 py-10 sm:grid-cols-[10rem_1fr] sm:gap-10 lg:py-16"
                delay={index * 0.1}
